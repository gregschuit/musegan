...
def setup():
    ...
    # Load parameters
    params = load_yaml(args.params)  # greg: args.params es el nombre del parameters_file yaml
...
def main():
    ...
    model = Model(params)
    if params.get('is_accompaniment'):
        # greg: train_c sería el compas del track condicionado, con una dimension extra al final (temporalidad?)
        train_c = tf.expand_dims(
            train_x[..., params['condition_track_idx']], -1)  # greg: Acá se añade una dimension extra al final del track a condicionar
        train_nodes = model(  # greg: model.__call__ crea los nodos del modelo.
    ...
    if config['save_samples_steps'] > 0:
        ...
        placeholder_z = tf.placeholder(tf.float32, shape=sample_z.shape)  # greg: un placeholder es como declarar un tensor vacio
        placeholder_y = None
        if params.get('is_accompaniment'):
            # greg: shape del placeholder es la misma que sample_x, pero la ultima dimension es 1.
            # greg: ¿ esto significa que c_shape contempla 1 solo track ?
